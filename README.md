...
3. **Smart scoring (optional, free tier)** — sends the shortlist's thumbnails
   ...
   There is no shared/server-side key: every person pastes in their own free
   Gemini key (stored only in their browser) before this stage will run, so
   it always counts against *their* quota, and getting a key is a deliberate
   extra step that separates it from the always-available private path.
...
5. That's it for setup — there's no Gemini key to configure here. Smart
   scoring is opt-in per person: anyone using it (including you) pastes their
   own free key (from [aistudio.google.com/apikey](https://aistudio.google.com/apikey))
   directly into the app at that step, every time they choose that path. This
   is deliberate — no shared server key exists to fall back to, so nobody's
   quota is silently shared with anyone else, and getting a key is a real
   speed bump between the always-free private path and the send-to-Google one.
