          ...
          <div className="mt-3 space-y-2 rounded border border-amber-400 bg-amber-100 p-3 text-xs text-amber-900 dark:border-amber-700 dark:bg-amber-900 dark:text-amber-200">
            ...
            <div>
              <label className="mb-1 block font-medium" htmlFor="gemini-key">
                Your own free Gemini API key (required — no shared key, by design)
              </label>
              <input
                id="gemini-key"
                type="password"
              ...
              <p className="mt-1 text-[11px] text-amber-700 dark:text-amber-400">
                ...
                for scoring). There is no fallback shared key — everyone brings their own,
                so this run always counts against <strong>your</strong> free quota, and
                getting a key is a deliberate extra step before any photo data leaves your
                browser.
              </p>
            </div>
            <button
            ...
            {!apiKey && (
              <p className="text-[11px] text-red-700 dark:text-red-400">
                Paste your own free key above to continue.
              </p>
            )}
          </div>
        )}
