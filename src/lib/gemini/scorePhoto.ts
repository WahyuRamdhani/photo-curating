...
async function callScoreApi(
  ...
  const res = await fetch("/api/score", {
    ...
    body: JSON.stringify({ imageBase64, mimeType, apiKey: apiKey || undefined }),
  });

  if (res.status === 429 && attempt < 4) {
    const backoffMs = 2000 * 2 ** attempt;
    await new Promise((r) => setTimeout(r, backoffMs));
    return callScoreApi(imageBase64, mimeType, apiKey, attempt + 1);
  }
...
export async function scorePhoto(
  ...
  try {
    ...
    const result = await callScoreApi(base64, mimeType, apiKey);
...
export async function scorePhotosSequentially(
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    const result = await scorePhoto(items[i].fileId, items[i].img, apiKey);
    results.push(result);
